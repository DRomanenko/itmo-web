...
public class UserRepositoryImpl implements UserRepository {
        ...
        for (int i = 1; i <= metaData.getColumnCount(); i++) {
            switch (metaData.getColumnName(i)) {
                ...
                case "creationTime":
                    ...
                    break;
                case "admin":
                    user.setAdmin(resultSet.getLong(i) == 1);
                    break;
                default:
                    // No operations.
            }
    ...
    public void setAdmin(long id, boolean admin) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE `User` SET `admin` = ? WHERE `User`.`id` = ?")) {
                statement.setLong(1, admin ? 1 : 0);
    ...
    public void save(User user, String passwordSha) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("INSERT INTO `User` (`login`, `passwordSha`, `admin`, `creationTime`) VALUES (?, ?, ?, NOW())", Statement.RETURN_GENERATED_KEYS)) {
                ...
                statement.setString(2, passwordSha);
                statement.setLong(3, user.isAdmin() ? 1 : 0);
                if (statement.executeUpdate() != 1) {
                    throw new RepositoryException("Can't save User.");
                } else {
