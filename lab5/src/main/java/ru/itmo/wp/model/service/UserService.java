...
public class UserService {
    ...
    public void validateRegistration(User user, String password, String passwordConfirmation) throws ValidationException {
        if (Strings.isNullOrEmpty(user.getLogin()) && Strings.isNullOrEmpty(user.getEmail())) {
            throw new ValidationException("Email and login are require");
        }
        ...
        if (!user.getEmail().matches("[^@]+@[^@]+(\\.[^@]+)+")) {
            //if (!user.getEmail().contains("@") && user.getEmail().indexOf("@", user.getEmail().indexOf("@") + 1) != -1) {
        ...
        if (Strings.isNullOrEmpty(user.getLogin())) {
            throw new ValidationException("Login is require");
        }
        ...
        if (userRepository.findByLoginAndEmail(user.getLogin(), user.getEmail()) != null) {
            throw new ValidationException("Login and Email are already in use");
        }
    ...
    public User validateAndFindByLoginOrEmailAndPassword(String loginOrEmail, String password) throws ValidationException {
        ...
        if (user == null) {
            user = userRepository.findByLoginOrEmailAndPasswordSha(loginOrEmail, getPasswordSha(password));
            if (user == null) {
