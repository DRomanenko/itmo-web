        ...
        components: {
            ...
            this.$root.$on("onRegister", (login, name) => {
                if (!login) {
                    ...
                    this.$root.$emit("onRegisterValidationError", "Length of login should be between 3 and 16 characters.");
                } else if (!login.match(/^[a-z]+$/)) {
                    this.$root.$emit("onRegisterValidationError", "Login should consist of lowercase Latin letters.");
                } else if (!name) {
                    this.$root.$emit("onRegisterValidationError", "Name is empty.");
                } else if (name.length > 32) {
                    this.$root.$emit("onRegisterValidationError", "Length of name should be between 1 and 32 characters.");
                } else {
                    let users = Object.values(this.users).filter(u => u.login === login);
                    if (users.length === 0) {
