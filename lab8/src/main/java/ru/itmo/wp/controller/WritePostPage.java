...
public class WritePostPage extends Page {
    ...
    public String writePostPost(@Valid @ModelAttribute("post") Post post,
        ...
        Post boii = new Post();
        boii.setText(post.getText());
        boii.setTitle(post.getTitle());
        String[] tags = post.getWrtags().trim().split("\\s+");
        Set<Tag> tagHashSet = new HashSet<>();
        ...
        for (String rawTag : tags) {
            try {
                Tag tag = postService.findTag(rawTag);
                tagHashSet.add(tag);
            } catch (Exception ignored) {
            }
        }
